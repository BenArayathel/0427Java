	...
	public static void main(String[] args) {
		...
		do {
			...
			case 2:
				...
				break;
			case 3:
				System.out.println("Enter ID of trainee to be updated");
				String id3=scanner.nextLine();
				System.out.println("Enter new contact number:");
				String contactString = scanner.nextLine();
				long newContact = Long.parseLong(contactString);
				try {
					Trainee trainee3 = service.updateTraineeContact(id3, newContact);
					if(trainee3 != null) {
						System.out.println("Trainee successfully updated");
					}
				...
				break;
			case 4:
				System.out.println("Enter ID of trainee to be deleted");
				String id4=scanner.nextLine();
				try {
					service.deleteTrainee(id4);
				...
				break;
			case 5:
				System.out.println("Enter city in which to search for trainees");
				String city = scanner.nextLine();
				List<Trainee> cityList = new ArrayList<>();
				try {
					cityList = service.getTraineesByCity(city);
				} catch (BusinessException e4) {
					System.out.println(e4.getMessage());
				}
				if(cityList != null && cityList.size() > 0) {
					for (Trainee t : cityList) {
