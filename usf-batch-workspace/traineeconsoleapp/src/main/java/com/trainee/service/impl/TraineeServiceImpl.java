...
public class TraineeServiceImpl implements TraineeService {
	...
	public Trainee updateTraineeContact(String id, long newContact) throws BusinessException {
		Trainee trainee = null;
		if(isValidId(id)) {
			trainee=dao.updateTraineeContact(id, newContact);
		} else {
			throw new BusinessException("Entered Id " + id + " is invalid");
		}
		return trainee;
	...
	public List<Trainee> getTraineesByCity(String city) throws BusinessException {
		List<Trainee> traineeList = dao.getTraineesByCity(city);
		if(traineeList == null || traineeList.size() == 0) {
			throw new BusinessException("No trainees from this city");
		}
		return traineeList;
	}
