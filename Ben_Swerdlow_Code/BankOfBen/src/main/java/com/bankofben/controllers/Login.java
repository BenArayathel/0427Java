...
public class Login {
	...
	public static String login(HttpServletRequest request, HttpServletResponse response) throws IOException, BusinessException {
		...
		if (testSession==null) {
			...
			if (testSession.getAttribute("customer")!=null) {
				Customer customer = (Customer) testSession.getAttribute("customer");
				if (customer.isApplicationPending()) {
					// if pending, send to pending
					respString = "/userPending.html";
				} else {
			...
			} else if (testSession.getAttribute("employee")!=null) {
//				Employee employee = (Employee) testSession.getAttribute("employee");
				respString = "/employee.html";
			} else {
				respString = "/userPending.html";
			}
