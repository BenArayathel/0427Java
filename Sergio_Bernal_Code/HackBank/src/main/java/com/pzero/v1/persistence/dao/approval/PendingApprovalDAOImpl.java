...
public class PendingApprovalDAOImpl implements PendingApprovalDAO{
	...
	@Override
	public boolean createApproval(PendingApproval pApproval) throws BusinessException{
		boolean flag = false;
...
//	public PendingApproval getApprovalById(int id) throws BusinessException{
//		PendingApproval pApproval = null;
//		String sql = "SELECT ID,  WHERE id = ?";
//		try(Connection conn = SingletonDBConnection.getConnection()){
//			PreparedStatement prepared = conn.prepareStatement(sql);
//			prepared.setString(1, pApproval.getStatus());
//			prepared.setInt(2, pApproval.getId());
