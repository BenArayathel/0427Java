				...
				if(intChoice == 9) {
					...
					case 1: 
						try {
							...
							loggy.info("Internal Error. Please try again");
							loggy.error("Error in signUpForAccount()" );
							e.printStackTrace();
						}
						break;
					case 2:
						if(accountCheck) {
							...
							try {
								String chkAmt= uSI.checkCheckingBalance(currentUser);
					...
					case 3:
						if(accountCheck) {
							try {
								String svgAmt = uSI.checkSavingsBalance(currentUser);
						...
						if (accountCheck) {
							...
							if (amt.matches("[0-9.]{1,7}")) {
								if (ch.equalsIgnoreCase("checking")) {
									try {
										uSI.depositMoney("checkingBalance", amt, currentUser);
									} catch (BusinessException e) {
									...
									try {
										uSI.depositMoney("savingsBalance", amt, currentUser);
									} catch (BusinessException e) {
										loggy.info("Error. Please try again");
